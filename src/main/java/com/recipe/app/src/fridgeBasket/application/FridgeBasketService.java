...
public class FridgeBasketService {
    ...
    public FridgeBasketService(FridgeBasketRepository fridgeBasketRepository, IngredientService ingredientService, IngredientCategoryService ingredientCategoryService) {
    ...
    @Transactional
    public void create(User user, FridgeBasketsRequest request) {
        ...
        Map<Long, FridgeBasket> fridgeBasketMapByIngredientId = getExistFridgeBasketMapByIngredientId(user.getUserId());
    ...
    private Map<Long, FridgeBasket> getExistFridgeBasketMapByIngredientId(Long userId) {

        List<FridgeBasket> existFridgeBaskets = findByUserId(userId);

        return existFridgeBaskets.stream()
                .collect(Collectors.toMap(FridgeBasket::getIngredientId, Function.identity()));
    }

    @Transactional
    public void delete(User user, Long fridgeBasketId) {
    ...
    @Transactional
    public void update(User user, Long fridgeBasketId, FridgeBasketRequest request) {

        FridgeBasket fridgeBasket = findByUserIdAndFridgeBasketId(user.getUserId(), fridgeBasketId);

        fridgeBasket.update(request.getExpiredAt(), request.getQuantity(), request.getUnit());
    ...
    @Transactional(readOnly = true)
    public FridgeBasketsResponse findAllByUser(User user) {
