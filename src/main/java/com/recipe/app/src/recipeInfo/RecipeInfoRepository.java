...
public interface RecipeInfoRepository extends CrudRepository<RecipeInfo, Integer> {
    //@Query("SELECT r FROM RecipeInfo r LEFT OUTER JOIN r.recipeIngredients ri on r.recipeId=ri.recipeInfo.recipeId WHERE (r.recipeNmKo LIKE CONCAT('%',:keyword,'%') OR ri.irdntNm LIKE CONCAT('%',:keyword,'%')) AND r.status=:status ")
    ...
    List<RecipeInfo> searchRecipeInfos(String keyword, String status, Pageable pageable);
}
