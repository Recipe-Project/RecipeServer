...
public interface ScrapBlogRepository extends CrudRepository<ScrapBlog, Integer> {
    ...
    //조회순
    @Query("SELECT b, count(v.blogUrl) From ScrapBlog b LEFT OUTER JOIN ViewBlog v ON b.blogUrl = v.blogUrl WHERE b.user.userIdx = :userIdx AND b.status = :status GROUP BY b.blogUrl ORDER BY count(v.blogUrl) DESC")
    ...
    //인기순(좋아요순)
    @Query("SELECT b, count(b2.blogUrl) From ScrapBlog b LEFT OUTER JOIN ScrapBlog b2 ON b.blogUrl = b2.blogUrl AND b2.status=:status WHERE b.user.userIdx = :userIdx AND b.status = :status GROUP BY b2.blogUrl ORDER BY count(b2.blogUrl) DESC")
